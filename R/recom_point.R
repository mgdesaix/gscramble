...
#' @param M a tibble that has the columns start_pos, end_pos, and theta (where theta is the
...
recomb_point <- function(M, always_one = TRUE) {
  if(always_one == TRUE) {  # in this case, always have one recombination. Just doing
    ...
    ival <- sample(1:nrow(M), size = 1, prob = M$theta) # get the map interval in which the recomb occurs
