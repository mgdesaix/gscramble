...
Let's develop this basic example of simulating F1 hybrids.  The first step would be to define the GSP in which we are using ‘p1’ and ‘p2’ in an associated `RepPop` to represent the two
...
We will fully describe `RepPop` tibbles below.  Given that GSPs need to be accompanied
by a `RepPop` tibble to define the groups to be hybridized, let's introduce `RepPops` by
specifying tibbles that accompany the GSPs of interest.   
...
So, producing F1s, F1BC1s, F2BC1s with gspComplex...
