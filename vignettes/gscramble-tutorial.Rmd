...
will get segregated across successive generations represented within the pedigree
...
1 up to the number of individuals in the pedigree). Founders are listed directly
...
This probably sounds a little abstract, and, indeed, it is.  `gscramble` includes
15 basic GSPs (illustrated within the object `GSP_opts`) that can be retreived by 
...
individuals from two population).  We then show the tibble representation of that
...
Now, for example, let’s say you are interested in simulating F1 hybrids for all pairwise combinations of populations (with the example data Pop1, Pop2, Pop3, Pop4).  F1s are symmetrical such that a Pop1-Pop2 hybrid is identical to a Pop2-Pop1 hybrid.  Accordingly, we need to specify the `RepPop` tibble to define which populations to use in each replicate of the simulation:
...
Next, we need to define the GSP in which we are using ‘p1’ and ‘p2’ in `RepPop` to represent the two
...
Building upon this simplistic GSP, let’s build something a little more ‘complex’ with F1s, F1BC1s, and F1BC2s using the built-in function `create_GSP()`.
...
For this example, Pop1 is our population of interest. We are simulating Pop1-Pop2 F1s, then pairing those F1 individuals with Pop1 to generate backcrossed hybrids, which are different than Pop1-Pop2, backcrossed to Pop2 hybrids. Here is a picture of what that looks like:
...
Now say we can simulate hybrid individuals between Pop1 and all other populations using the `RepPop` tibble.


```{r}
Pattern = c("Pop1", "Pop2", "Pop1",
            "Pop3", "Pop1", "Pop4")
RepPopComplex <- tibble(
  rep = rep(1:3, each = 2),
...
```{r}
Pattern = c("Pop2", "Pop1", "Pop3", 
            "Pop1", "Pop4", "Pop1")
RepPopComplex2 <- tibble(
  rep = rep(1:3, each = 2),
  pop = rep(c("p1", "p2"), times = 3), 
  group = Pattern
)
...
to map "A" to Pop2 and "B" to Pop3, and in the second rep we might want to map "A" to Pop1, and "B" to
Pop 4.  (Note, at this point, genetic material from 3 individuals from each of those populations will have
...
RepPop4
```
Note that this request will consume 1 individual from each of populations 2, 3, 4, and 10 (which are mapped to A, B, C, and D, respectively),
and will create 4 simulated, admixed, and sampled individuals.
...
to the function call, we get little sparklines shows us the recombination rates across the
chromosomes.
...
* `$ret_geno` is an N x 2L matrix of returned genotypes. The two alleles at each
...
* `$ret_ids`: a tibble with IDs of the individuals corresponding to the rows
    in the `ret_geno` matrix.  There are two columns: `group` and `indiv`.  The first
    rows include the samples from the GSP.  Their group is `ped_hybs` (meaning
    ...
    Those are (they need explaining):
    - `rep`:
    - `ped`:
    - `sample_id`:
    - `samp_index`:
...
## Reading from plink format {#readplink}

If the marker data you wish to use for simulation in gscramble is in PLINK format,
the function `plink2gscramble()` will read a `.map` and `.ped` file pair
into a list of objects like `Geno`, `I_meta`, and `M_meta`.

Note that this function can handle gzipped files, as well (i.e., `.map.gz` and `.ped.gz`).
However, it cannot read PLINK binary files (i.e., `.bim`, `.bed` files).

Its use is as follows, reading some example data from the package:
```{r}
mapfile <- system.file("extdata/example-plink.map.gz", package = "gscramble")
pedfile <- system.file("extdata/example-plink.ped.gz", package = "gscramble")

GS_data <- plink2gscramble(mapfile, pedfile)
