...
to make them small enough to host on CRAN) from a data set of feral pigs (CITATION) are included as data objects with the 
...
to indicate how many gametes must be segregated to each individuals (from each of its
two parents) in order to consume all the genetic material present in the founders.

This probably sounds a little abstract, and, indeed, it is.  To try to explain the
GSP concept more fully, we will first draw a picture of one, and then show the tibble
representation of that same GSP and discuss the structure of a GSP in light of those
two pieces of information.


The package data object `GSP` shows an example of a genome simulation pedigree.  It is
a fairly complex pedigree with 13 members.  Here is a picture of what it looks like:
...
but it requires the installation of the GraphViz `dot` software.
```{r eval=FALSE}
