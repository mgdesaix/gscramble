...
Building up this pattern of hybridization between Pop1 and Pop2, now we can simulate hybrid individuals between Pop1 and all other populations using the `RepPop` tibble.
...
However, the actual group names used in your genotype data set might be different. In our example
data for this R package, we have 4 groups, with individuals associated with each of
those groups as defined in `I_meta`.  To illustrate how many individuals are included
in each of these groups in the example data:
...
6 of which are founders: 3 from population "A" and 3 from population "B").  For the first index, we might want
to map "A" to Pop1 and "B" to Pop2, and in the second index we might want to map "A" to Pop4, and "B" to
...
* `$ret_ids`: a tibble with IDs of the individuals corresponding to the rows
    ...
    format of an `h-` followed by a string with replaced values of: `gpp-index-ped_sample_id-samp_index-matrix_row`.
