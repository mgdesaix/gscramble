...
are included as data objects with the package. The full data set, from
...
procedure as defined in the `index` column (1:nReplicates). (Note that the `index` column
...
dim(plinkPRE$M_meta)
```
Where, we use the `gz_ext = TRUE` option because here, our files are gzipped with the `.gz` extension.
