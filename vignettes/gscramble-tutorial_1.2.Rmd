...
let's say they are denoted as `"-1"`, then you can change all missing values to `NA`s using the following:

```{r, eval=FALSE}
Geno[Geno == "-1"]<-NA
```

Since there are no `"-1"` or `NA`s in the `Geno` dataset, nothing happens.
...
`"1"`, `"X"` of `"Omy28"`, or as illustrated in `M_meta` chr12, chr17, and chr18.  
...
Now, for example, let’s say you are interested in simulating F1 hybrids for all pairwise combinations of populations (with the example data Pop10, Pop2, Pop3, Pop4).  F1s are symmetrical such that a Pop10-Pop2 hybrid is identical to a Pop2-Pop10 hybrid.  Accordingly, we need to specify the `RepPop` tibble:

```{r eval=FALSE}
Pattern = c("Pop10", "Pop2", "Pop10", "Pop3", "Pop10", "Pop4", "Pop2", "Pop3", "Pop2", "Pop4", "Pop3", "Pop10")
...
Next, we need to define the GSP in which we are using ‘p1’ and ‘p2’ in `RepPop` to represent Pop1 and Pop2.  
```{r eval=FALSE}
gspF1 <- create_GSP(pop1 = "p1", pop2 = "p2", F1 = TRUE, F2 = FALSE, F1B = FALSE, F1B2 = FALSE)
```

Now, concatenate ‘RepPopF1’ with ‘gspF1’

```{r eval=FALSE}
Input_tibble_F1 <- tibble(
  gpp = list(gspF1),
  reppop = list(RepPopF1)
)
# And create segments
set.seed(15) # for reproducibility
Segments_ts <- segregate(
  request = Input_tibble_F1,
  RR = RecRates
)
dim(Segments_ts)
```

Building upon this simplistic GSP, let’s build something a little more ‘complex’ with F1s, F1BC1s, and F1BC2s.

```{r eval=FALSE}
gspComplex <- create_GSP(pop1 = "p1", pop2 = "p2", F1 = TRUE, F2 = FALSE, F1B = TRUE, F1B2 = TRUE)
```

With this illustration, Pop10 is our population of interest. Accordingly, we are interested in all combinations with Pop10 but not necessarily among the other populations.  Notice that backcrossed individuals are not symmetrical.  Thus, with this example, we are simulating Pop10-Pop2 F1s, then paired with Pop10 to generate backcrossed hybrids, which are different than Pop10-Pop2, backcrossed to Pop2 hybrids.

```{r eval=FALSE}
Pattern = c("Pop10", "Pop2", "Pop10", "Pop3", "Pop10", "Pop4")
...
As before, concatenate these two tibbles and then define segments

```{r eval=FALSE}
Input_tibble_Complex <- tibble(
  gpp = list(gspComplex),
  reppop = list(RepPopComplex)
)

set.seed(15) # for reproducibility
SegmentsComplex <- segregate(
  request = Input_tibble_Complex,
  RR = RecRates
)
...
Again, we specified an initial suite of GSPs that were of interest, combining various configurations of F1s, F2s, F1BC1s, and F1BC2s.  These can be generated or called with the `create_GSP()`.  However, you are not limited to these GSPs.  
...
in the 'gscramble' package, but it requires the installation of the GraphViz `dot` software.
